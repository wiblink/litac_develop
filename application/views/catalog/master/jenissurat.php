...
<div class="content">
              ...
              <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                 ...
                 if ($userData['kewenangan'] == 'admin') { ?>
                    <br>
                    <a href="jenissurat/t" class="btn btn-primary d-none d-sm-inline-block">
           ...
           <div class="col-12">
              <div class="card">
                ...
                <div class="table-responsive">				
                  <table id="jenistable" class="table card-table table-vcenter text-nowrap datatable">
                    <thead>
                      <tr>
						  <th width="30px;">No.</th>
						  <th>Jenis Surat</th>
		...
		{
			var table = jQuery("#jenistable").DataTable({
			ScrollX   : false ,
			serverSide : true,
			processing : true,       
			ajax       : {
				method : "GET",
				url  : "<?php echo base_url('ketatausahaan/jenissurat_json') ?>",
				data : data => {
					// Read values
			...
			columns       : [
							...
							{ data: "jenis_surat", orderable: true },
		...
		function fill_search_datatable(search = '', searchtext = '')
		  {
		   var dataTable = $('#jenistable').DataTable({
			processing : true,
			serverSide : true,
			//"order" : [],
			ajax : {
			 url  : "<?php echo base_url('ketatausahaan/search_jenissurat_json') ?>",
			 type:"POST",
			 data:{
	...
	function eAlert(x){
		...
		.then((willDelete) => {
			if (willDelete) {
				swal("Sukses! Jenis Surat berhasil dihapus.", {
					icon: "success"
				});
				window.location.href = '../ketatausahaan/jenissurat/h/'+ x +'';
			} else {
				swal("Jenis Surat batal dihapus");
			}
		});
	}
