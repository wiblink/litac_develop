...
class Form_builder {
    ...
    private $config = array(/* Config array - can be overrided by passing in array in ini() */
        ...
        'default_date_today_if_not_set' => TRUE,
        'default_datepicker_class' => 'date-picker', // For instance 'date-picker'
...
//        }
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_a':
                    ...
                    $input_html_string = anchor($this->elm_options['href'], $value, $this->elm_options);
                    break;
                case 'form_label':
                ...
                case 'form_date':
                    ...
                    $input_html_string = form_input($this->elm_options,(in_array('value',array_keys($this->elm_options)) ? $this->elm_options['value'] : ''),$this->_create_extra_ext_string());
                    break;
                case 'form_email':
                ...
                case 'form_submit':
                    ...
                    $input_html_string = form_submit($name, $label, $this->_create_extra_string($this->elm_options).' '.$this->_create_extra_ext_string());
                    break;
                case 'form_option':
                case 'form_dropdown':
                    ...
                    if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
						...
						if (isset($this->elm_options['multiple']) && !empty($this->elm_options['multiple'])) $input_html_string = form_dropdown($name, $options, $value, $this->_create_extra_string()." multiple='multiple'");
                        else $input_html_string = form_dropdown($name, $options, $value, $this->_create_extra_string());
                    } else {
                        dump($this->elm_options);
                ...
                case 'form_textarea':
                    ...
                    $input_html_string = form_textarea($this->elm_options,(in_array('value',array_keys($this->elm_options)) ? $this->elm_options['value'] : ''),$this->_create_extra_ext_string());
                    break;
                case 'form_file':
	...
	private function _create_extra_ext_string() {
        $extrakeys = array('rules');
		$extra = '';
        foreach ($extrakeys as $k) {
            if (in_array($k,array_keys($this->elm_options))) $extra .= " {$this->elm_options[$k]} ";
        }
        return trim($extra);
    }
